...
const CardChooserView = ({ cardSettings, cardLibrary, save }) => {
  ...
  const cardSettingsCopy = JSON.parse(cardSettings[0].cardSettings)
  ...
  const checkCardSettings = JSON.parse(cardSettings[0]?.cardSettings)
    ? JSON.parse(cardSettings[0]?.cardSettings)
    : []

  const cardSettingCardList = checkCardSettings?.length
    ? [...checkCardSettings]
    : defaultCardList
  ...
  const allOn = () => {
    const newState = cardLibraryCopy?.map((card) => {
      return { ...card, isChecked: true, isVisible: true }
    })
    setCheckState(newState)
    return
